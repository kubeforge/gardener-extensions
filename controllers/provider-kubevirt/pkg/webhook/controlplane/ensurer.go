...
func ensureKubeAPIServerCommandLineArgs(c *corev1.Container) {
	c.Command = controlplane.EnsureStringWithPrefix(c.Command, "--cloud-provider=", "kubevirt")
...
func ensureKubeControllerManagerCommandLineArgs(c *corev1.Container) {
	...
	c.Command = controlplane.EnsureStringWithPrefix(c.Command, "--cloud-config=",
		"/etc/kubernetes/cloudprovider/cloudprovider.conf")
	c.Command = controlplane.EnsureStringWithPrefix(c.Command, "--external-cloud-volume-plugin=", "kubevirt")
}
	...
	cloudProviderConfigVolume = corev1.Volume{
		...
		VolumeSource: corev1.VolumeSource{
			ConfigMap: &corev1.ConfigMapVolumeSource{
				LocalObjectReference: corev1.LocalObjectReference{Name: kubevirt.CloudProviderConfigName},
			},
		},
	}
)
...
func (e *ensurer) ensureChecksumAnnotations(ctx context.Context, template *corev1.PodTemplateSpec, namespace string) error {
	if err := controlplane.EnsureConfigMapChecksumAnnotation(ctx, template, e.client, namespace, kubevirt.CloudProviderConfigName); err != nil {
		return err
	}
	return nil
}
...
func ensureKubeletCommandLineArgs(command []string) []string {
	command = controlplane.EnsureStringWithPrefix(command, "--cloud-provider=", "kubevirt")
	return command
}
