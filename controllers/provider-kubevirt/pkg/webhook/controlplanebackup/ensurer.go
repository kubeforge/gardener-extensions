...
func NewEnsurer(etcdBackup *config.ETCDBackup, imageVector imagevector.ImageVector, logger logr.Logger) genericmutator.Ensurer {
	return &ensurer{
		...
		logger:      logger.WithName("kubevirt-controlplanebackup-ensurer"),
	}
}
...
func (e *ensurer) ensureChecksumAnnotations(ctx context.Context, template *corev1.PodTemplateSpec, namespace, name string) error {
	if name == common.EtcdMainStatefulSetName {
		return controlplane.EnsureSecretChecksumAnnotation(ctx, template, e.client, namespace, kubevirt.BackupSecretName)
	}
	return nil
}
...
func (e *ensurer) getBackupRestoreContainer(name string, cluster *extensionscontroller.Cluster) (*corev1.Container, error) {
	...
	image, err := e.imageVector.FindImage(kubevirt.ETCDBackupRestoreImageName, "", cluster.Shoot.Spec.Kubernetes.Version)
	if err != nil {
		return nil, errors.Wrapf(err, "could not find image %s", kubevirt.ETCDBackupRestoreImageName)
	}
	...
	if name == common.EtcdMainStatefulSetName {
		provider = kubevirt.StorageProviderName
		env = []corev1.EnvVar{
			{
