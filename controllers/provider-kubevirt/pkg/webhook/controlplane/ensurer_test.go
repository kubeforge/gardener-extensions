...
func TestController(t *testing.T) {
	...
	RunSpecs(t, "kubevirt Controlplane Webhook Suite")
}
...
var _ = Describe("Ensurer", func() {
	var (
		...
		cmKey    = client.ObjectKey{Namespace: namespace, Name: kubevirt.CloudProviderConfigCloudControllerManagerName}
		cmKCMKey = client.ObjectKey{Namespace: namespace, Name: kubevirt.CloudProviderConfigKubeControllerManagerName}
		cm       = &corev1.ConfigMap{
			ObjectMeta: metav1.ObjectMeta{Namespace: namespace, Name: kubevirt.CloudProviderConfigCloudControllerManagerName},
			Data:       map[string]string{"abc": "xyz", kubevirt.CloudProviderConfigMapKey: cloudProviderConfigContent},
		}

		annotations = map[string]string{
			"checksum/configmap-" + kubevirt.CloudProviderConfigCloudControllerManagerName: "2ac8b96caad089f7b0217f0b2916ff4e8d4346655746de55178207e180cf0bbe",
		}
								...
								Containers: []corev1.Container{
									{
										...
										VolumeMounts: []corev1.VolumeMount{
											{Name: kubevirt.CloudProviderConfigKubeControllerManagerName, MountPath: "?"},
										},
									},
								},
...
func checkKubeAPIServerDeployment(dep *appsv1.Deployment, annotations map[string]string) {
	...
	Expect(c.Command).To(ContainElement("--cloud-provider=kubevirt"))
...
func checkKubeControllerManagerDeployment(dep *appsv1.Deployment, annotations, labels map[string]string) {
	...
	Expect(c.Command).To(ContainElement("--cloud-config=/etc/kubernetes/cloudprovider/cloudprovider.conf"))
	Expect(c.Command).To(ContainElement("--external-cloud-volume-plugin=kubevirt"))
	Expect(c.VolumeMounts).To(ContainElement(cloudProviderConfigKubeControllerManagerVolumeMount))
